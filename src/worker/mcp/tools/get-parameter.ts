...
import { findParameter } from '../utils/search';
...
export function getParameter(
  ...
  try {
    ...
    const found = findParameter(name, componentsData, component);

    if (!found) {
      return toolError(
        `Parameter "${name}" not found${component ? ` in component "${component}"` : ''}`
      );
    }
